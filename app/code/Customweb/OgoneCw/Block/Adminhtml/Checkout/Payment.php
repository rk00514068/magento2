...
namespace Customweb\OgoneCw\Block\Adminhtml\Checkout;
...
class Payment extends \Magento\Backend\Block\Widget\Form\Container
{
	 ...
	 * @var string
	 */
	protected $_blockGroup = 'Customweb_OgoneCw';
	...
	protected function _construct()
	{
		...
		$this->setId('ogonecw_checkout_payment');

		$this->buttonList->update('save', 'label', __('Submit Order'));
		$this->buttonList->update('save', 'class', 'primary');
		$this->buttonList->update('save', 'id', 'ogonecw-payment-submit-button');
		...
		$this->buttonList->add(
			...
			[
				'label' => __('Cancel'),
				'class' => 'cancel',
				'id' => 'ogonecw-payment-cancel-button'
			]
		);
	}
	...
	public function getTransaction()
	{
		if (!($this->transaction instanceof \Customweb\OgoneCw\Model\Authorization\Transaction)) {
			...
			if (!($transaction instanceof \Customweb\OgoneCw\Model\Authorization\Transaction) || !$transaction->getId()) {
