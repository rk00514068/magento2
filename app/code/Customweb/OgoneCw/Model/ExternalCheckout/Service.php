...
class Service implements \Customweb_Payment_ExternalCheckout_ICheckoutService
{
	...
	/**
	 * @var \Customweb\OgoneCw\Helper\ExternalCheckout
	 */
	protected $_helper;
	 ...
	 * @param \Customweb\OgoneCw\Helper\ExternalCheckout $helper
	...
	public function __construct(
			...
			\Customweb\OgoneCw\Helper\ExternalCheckout $helper,
	...
	/**
	 * @param \Customweb\OgoneCw\Model\DependencyContainer $container
	 * @return \Customweb\OgoneCw\Model\ExternalCheckout\Service
	 */
	public function setContainer(\Customweb\OgoneCw\Model\DependencyContainer $container)
	{
	...
	{
		if (!($context instanceof \Customweb\OgoneCw\Model\ExternalCheckout\Context)) {
			throw new \Customweb_Core_Exception_CastException('Customweb\OgoneCw\Model\ExternalCheckout\Context');
		}
...
// TODO: Implement a real check here. This method was added to avoid a DI compile error.
	...
	public function authenticate(\Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl)
	{
		...
		if ($this->_checkoutHelper->isAllowedGuestCheckout($context->getQuote())
				&& $this->_scopeConfig->getValue('ogonecw/shop/external_checkout_account_creation') == 'skip_selection') {
			...
			$context->setRegisterMethod(\Customweb\OgoneCw\Model\ExternalCheckout\Context::REGISTER_METHOD_GUEST);
		...
		$context->save();
		return \Customweb_Core_Http_Response::redirect($context->getQuote()->getStore()->getUrl('ogonecw/externalCheckout/login', ['_secure' => true]));
	}
	...
	public function renderShippingMethodSelectionPane(\Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages)
	{
		...
		return $this->_layoutFactory->create()->createBlock('Customweb\OgoneCw\Block\ExternalCheckout\ShippingMethods')
			->setContext($context)
	...
	public function renderReviewPane(\Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage)
	{
		...
		$layout->getUpdate()->load(['ogonecw_externalcheckout_review']);
		$layout->generateXml();
		$layout->generateElements();
		$layout->getBlock('customweb_ogonecw_externalcheckout_review')
			->setContext($context)
	...
	public function createOrder(\Customweb_Payment_ExternalCheckout_IContext $context)
	{
		...
		try {
			...
			$transaction = $this->_coreRegistry->registry('ogonecw_transaction');
