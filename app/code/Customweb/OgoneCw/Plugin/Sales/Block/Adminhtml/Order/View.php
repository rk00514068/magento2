...
namespace Customweb\OgoneCw\Plugin\Sales\Block\Adminhtml\Order;
...
class View
{
	...
	public function aroundGetReviewPaymentUrl(\Magento\Sales\Block\Adminhtml\Order\View $subject, \Closure $proceed, $action)
    {
    	if ($subject->getOrder() instanceof \Magento\Sales\Model\Order
	    		...
	    		&& $subject->getOrder()->getPayment()->getMethodInstance() instanceof \Customweb\OgoneCw\Model\Payment\Method\AbstractMethod) {
		    ...
		    return $subject->getUrl('ogonecw/transaction/reviewPayment', ['action' => $action, 'transaction_id' => $transaction->getId()]);
    	} else {
    ...
    {
		$this->_coreRegistry->register('ogonecw_order_view', true, true);
    }
}